...
public class KOM_CommandLoader
{
    public static final Pattern CJFM_COMMAND_CLASS_PATTERN = Pattern.compile(TotalFreedomMod.KOM_COMMAND_PATH.replace('.', '/') + "/(" + TotalFreedomMod.KOM_COMMAND_PREFIX + "[^\\$]+)\\.class");
    private List<TFM_CommandInfo> commandList = null;

    private KOM_CommandLoader()
    {
    }
            ...
            if (codeSource != null)
            {
                ...
                while ((zipEntry = zip.getNextEntry()) != null)
                {
                    ...
                    if (matcher.find())
                    {
                        try
                        {
                            Class<?> commandClass = Class.forName(TotalFreedomMod.KOM_COMMAND_PATH + "." + matcher.group(1));
    ...
    public static KOM_CommandLoader getInstance()
    {
    ...
    {
        private static final KOM_CommandLoader INSTANCE = new KOM_CommandLoader();
    }
}